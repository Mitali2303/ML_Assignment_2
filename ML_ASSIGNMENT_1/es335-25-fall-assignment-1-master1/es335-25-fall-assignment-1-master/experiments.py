...
def create_data(N, P, data_type='binary', output_type='discrete'):
    if data_type == 'binary':
...
def test_runtime(N_values, P_values, data_type, output_type):
    avg_train_times = []
    avg_predict_times = []
    ...
    for N in N_values:
        for P in P_values:
            ...
            for _ in range(num_average_time):
                # Generate data
                X, y = create_data(N, P, data_type=data_type, output_type=output_type)
...
def plot_results(N_values, P_values, train_times, predict_times, case_description):
    ...
    plt.pause(30)            #  keeps plot visible for 30 sec
    plt.close(fig)          #  closes so next case can plot
...
for data_type, output_type, description in cases:
    ...
    avg_train_times, avg_predict_times = test_runtime(N_values, P_values, data_type, output_type)
